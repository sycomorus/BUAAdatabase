            ...
            <a-row :gutter="20">
              <a-col :span="10">
                <a-input placeholder="电话号码（必填）"
                  v-decorator="['teltphoneNumber', { rules: [{ required: true, message: ' ', whitespace: true }], validateTrigger: 'onSubmit' }]" />
              </a-col>
              <a-col :span="14">
...
<script>
import {sendPost} from '@/services/user'
    ...
    handleSubmit() {
      ...
      this.postForm.validateFields((errors, values) => {
        if (!errors) {
          ...
          sendPost(title, startDate, endDate, subjects, location, fullLocation, teltphoneNumber, emailAddress, content).then(this.afterPost)
        } else {
