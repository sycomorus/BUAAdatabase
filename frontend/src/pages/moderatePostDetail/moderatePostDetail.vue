    ...
    <page-layout :title="post.title" :desc="post.startDate + ' 至 ' + post.endDate">
        <a-card class="post-detail-card">
            ...
            <a-space wrap class="button-container">
                <a-button v-if="$route.params.showModerate !== false" type="primary" @click="handleApprove">通过</a-button>
                <a-button v-if="$route.params.showModerate !== false" type="primary" @click="handleReject">驳回</a-button>
...
import { getPost, approvePost, rejectPost } from '@/services/user'

export default {
    name: 'moderatePostDetail',
        ...
        handleApprove() {
            approvePost(this.post.id).then(response => {
                if (response.data.code >= 0) {
                    ...
                    this.$message.error('帖子通过失败');
                }
            }).catch(error => {
                console.error('帖子通过失败:', error);
            });
            this.goBack();
        },
        handleReject() {
            rejectPost(this.post.id).then(response => {
                if (response.data.code >= 0) {
                    ...
                    this.$message.error('帖子驳回失败');
                }
            }).catch(error => {
                console.error('帖子驳回失败:', error);
            });
            this.goBack();
        },
        goBack() {
