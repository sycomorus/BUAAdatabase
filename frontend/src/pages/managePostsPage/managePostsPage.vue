        ...
        <a-list itemLayout="vertical">
          <a-list-item v-for="(post, index) in posts" :key="index" class="list-item" @click="navigateToPost(post.id)">
            ...
            <div class="content">
              ...
              <div class="author" @click="getRole($event, post.id)">
                <router-link v-if="userRole === '家教'" :to="{ name: '家教主页', params: { id: post.authorId } }">
                  {{ post.author }}
                </router-link>
                <router-link v-else-if="userRole === '学生'" :to="{ name: '学生主页', params: { id: post.authorId } }">
                  {{ post.author }}
                </router-link>
                <em>{{ post.date }}</em>
                <em>{{ post.location }}</em>
      ...
      getUserRole(postId)
        .then(response => {
          if (response.data.code >= 0) {
            ...
            console.error('获取用户身份失败:', response.data.msg);
          }
        })
        .catch(error => {
