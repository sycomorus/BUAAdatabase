...
```json
{
	"code": int   // 成功则返回0，失败则返回1，比如当oldPassword不匹配时返回1
	"message":""   // 失败时返回错误信息，成功时返回空即可
}
```
...
```json
{
	"code": 0,   // 成功返回0，否则返回-1
    "data": {
        "announcements":[{   // 一个包含通知信息的列表
            "title": "",     // 通知标题
            "content": "",   // 通知内容
            "date": ""       // 通知发布日期，若在24小时内显示"x小时前"，在3天内显示"x天前"，否则显示yyyy-mm-dd
        }]
    }
}
```
